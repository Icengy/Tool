...
@interface ZZCameraPickerViewController()<UICollectionViewDelegate,UICollectionViewDataSource,ZZCameraFocusDelegate,ZZCameraBrowerDataSource, AVCapturePhotoCaptureDelegate>
...
- (void)makeTopViewUI
{
    UIView *topView = [[UIView alloc]initWithFrame:CGRectMake(0, StatusBar_Height, ZZ_VW, 44)];
    if (_themeColor) {
    ...
    flashBtn.tag = 2015;
    if (self.outputSettings.flashMode == 0) {
        [flashBtn setImage:Flash_close_Btn_Pic forState:UIControlStateNormal];
    }else if(self.outputSettings.flashMode == 1){
...
- (void)makeDownViewUI
{
    ...
    _downView = [[UIView alloc]initWithFrame:CGRectMake(0, ZZ_VH - 64 - photoSize - 20 - SafeAreaBottomHeight, ZZ_VW, 64 + photoSize + 20)];
    if (_themeColor) {
...
- (void)initCameraMain
{
    ...
    [self.captureOutput setPhotoSettingsForSceneMonitoring:self.outputSettings];
...
//    [self.captureOutput setPhotoSettingsForSceneMonitoring:self.outputSettings];
//
//    [self.captureOutput capturePhotoWithSettings:self.outputSettings delegate:self];
    ...
    if (btn.tag == 2015) {
        if (self.outputSettings.flashMode == 0) {
            [self flashLightModel:^{
                [self.outputSettings setFlashMode:AVCaptureFlashModeOn];
            }];
            [btn setImage:Flash_Open_Btn_Pic forState:UIControlStateNormal];
            
        }else if (self.outputSettings.flashMode == 1){
            [self flashLightModel:^{
                [self.outputSettings setFlashMode:AVCaptureFlashModeOff];
            }];
