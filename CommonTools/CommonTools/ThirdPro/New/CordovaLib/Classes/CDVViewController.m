...
- (void)printVersion
{
    DLog(@"Apache Cordova native platform version %@ is starting.", CDV_VERSION);
}

- (void)printPlatformVersionWarning
{
    if (!IsAtLeastiOSVersion(@"6.0")) {
        DLog(@"CRITICAL: For Cordova 3.5.0 and above, you will need to upgrade to at least iOS 6.0 or greater. Your current version of iOS is %@.",
...
- (void)printMultitaskingInfo
{
    ...
    DLog(@"Multi-tasking -> Device: %@, App: %@", (backgroundSupported ? @"YES" : @"NO"), (![exitsOnSuspend intValue]) ? @"YES" : @"NO");
}
...
- (void)loadSettings
{
    ...
    if (configParser == nil) {
        DLog(@"Failed to initialize XML parser.");
        return;
    }
...
- (void)didReceiveMemoryWarning
{
    ...
    while ((plugin = [enumerator nextObject])) {
        if (plugin.hasPendingOperation) {
            DLog(@"Plugin '%@' has a pending operation, memory purge is delayed for didReceiveMemoryWarning.", NSStringFromClass([plugin class]));
            doPurge = NO;
        }
    }
...
{
    DLog(@"Resetting plugins due to page load.");
...
{
    DLog(@"Finished load of: %@", theWebView.request.URL);
...
- (id)getCommandInstance:(NSString*)pluginName
{
    ...
    if (!obj) {
        ...
        if (obj != nil) {
            ...
            DLog(@"CDVPlugin class %@ (pluginName: %@) does not exist.", className, pluginName);
        }
    }
    return obj;
}
